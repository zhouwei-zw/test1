...
public class TestcmController {
    ...
    @GetMapping("/ljgeneratetoken1")
    public static String ljgeneratetoken1() {
        Map<String, Object> header = new HashMap<>();
...
//        date2 = ca.getTime();
//        String sdate = formatter.format(date2);
//        long currentTime= (long) (Timestamp.valueOf(sdate).getTime());
//        System.out.println(currentTime);

        Map<String,Object> claims1=new HashMap<>();
        claims1.put("exp",currentTime);
        claims1.put("client_id","big_screen_ksljz");
        claims1.put("client_type",3);
        String jsonStr = JSONObject.toJSONString(claims1);
        //秘钥
        String secret1="krcPjKYFfCMY8fTbGmiU2aP5wDbcS1cFhRp1kdhA";
        String token="";
        try {
            JwtBuilder builder=Jwts.builder();
            builder.setHeader(header);
            builder.setPayload(jsonStr);
            byte[] signkey= DatatypeConverter.parseBase64Binary(secret1);
            builder.signWith(SignatureAlgorithm.HS256,signkey);
            token=builder.compact();
            System.out.println(token);
    ...
    @GetMapping("/ljgeneratetoken2")
    public static String ljgeneratetoken2() {
        Map<String, Object> header = new HashMap<>();
        header.put("alg", "HS512");
        header.put("typ", "JWT");
